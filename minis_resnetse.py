...
def test_model(model, name):
    ...
    logger.info(f"Accuracy of {name} : {accuracy:.2f}%")  # 使用logger记录准确率
    logger.info(f"Recall of {name} : {recall:.2f}%")  # 使用logger记录召回率

    # 绘制混淆矩阵
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=trainset.classes)
    disp.plot(cmap=plt.cm.Blues)
    plt.title(f"Confusion Matrix of {name}")
    plt.savefig(f"{name}_confusion_matrix.png")  # 保存图片
    plt.show()
...
if __name__ == '__main__':
    ...
    # 配置日志记录器
    log_file = os.path.join(log_dir, "train_minis_resnetse.log")
    logging.basicConfig(
    ...
    # 训练完成后保存模型
    model_path = "./model/mnist_resnet.pth"
    torch.save(model.state_dict(), model_path)
    logger.info(f"模型已保存到 {model_path}")

    # 评估模型
    test_model(model, "minis_resnetse")
